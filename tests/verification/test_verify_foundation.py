...
# Import the actual VectorStore implementation and database connection
...
async def verify_foundation():
    ...
    methods = [
        m for m in dir(store) if not m.startswith("_") and callable(getattr(store, m))
    ]
    ...
    try:
        ...
        for row in schema_rows:
            ...
            print(
                f"    - {row['column_name']}: {type_str} {nullable_str} {default_str}"
            )
            ...
            try:
                ...
                if dim_result:
                    ...
                    if dimensions == 384:
                        ...
                        print(
                            f"  ❌ ERROR: Expected 384 dimensions, found {dimensions}"
                        )
    ...
    try:
        ...
        content_id = await store.store_content(
            content_type="test", source_path=test_path, content=test_content
        )
        ...
        if len(chunks) == 2:
            ...
            for chunk in chunks:
                print(
                    f"     - Chunk {chunk['chunk_index']}: embedding ID {chunk['id']}"
                )
            results["multiple_chunks"] = True
        else:
            print(f"  ❌ ERROR: Expected 2 chunks, found {len(chunks)}")
            results["errors"].append(
                f"Multiple chunks test failed: found {len(chunks)} chunks"
            )
    ...
    print(
        f"✓ Vector Dimensions: {results['vector_config'].get('dimensions', 'Unknown')}"
    )
    print(
        f"✓ Multiple Chunks Support: {'✅ PASS' if results['multiple_chunks'] else '❌ FAIL'}"
    )
...
async def main():
    """Main entry point that initializes database connection."""
    # Initialize database connection from environment
    config = DatabaseConfig.from_env()
    await init_database_connection(config)

    # Run verification
    await verify_foundation()


if __name__ == "__main__":
    # Run the verification with database initialization
    asyncio.run(main())
